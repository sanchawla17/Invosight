...
import { createShareLink, disableShareLink, fetchInvoiceById, updateInvoice } from "../../api/invoiceApi";
...
const InvoiceDetail = () => {
  ...
  const handleShareLink = async () => {
    try {
      ...
      setShareUrl(url);
      setInvoice((prev) => (prev ? { ...prev, shareEnabled: true } : prev));
      const copied = await copyToClipboard(url);
      toast.success(
        copied ? "Share link copied to clipboard." : "Share link created."
      );
    } catch (error) {
  ...
  const handleCopyShareLink = async () => {
    ...
    try {
      ...
      toast.success(
        copied ? "Share link copied to clipboard." : "Copy not supported."
      );
    } catch (error) {
  ...
  const handleDisableShareLink = async () => {
    try {
      ...
      setInvoice((prev) => (prev ? { ...prev, shareEnabled: false } : prev));
      setShareUrl("");
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="secondary" onClick={handleShareLink} isLoading={isSharing}>Share Invoice</Button>
          {invoice.shareEnabled && (
            <Button
              variant="ghost"
              onClick={handleDisableShareLink}
              isLoading={isDisablingShare}
            >
              Disable Share Link
            </Button>
          )}
      ...
      {shareUrl && (
        <div className="mb-6 print:hidden flex justify-end">
          <Button variant="ghost" size="small" onClick={handleCopyShareLink}>
            Copy Link
          </Button>
        </div>
      )}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start pb-8 border-b border-slate-200">
            ...
            <div className="text-left sm:text-right mt-4 sm:mt-0">
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                getComputedStatus(invoice) === 'Paid' ? 'bg-emerald-100 text-emerald-800' : 
                getComputedStatus(invoice) === 'Overdue' ? 'bg-red-100 text-red-800' :
